...
We've already seen how StratoWeave can hold intent at multiple layers of abstraction, and how it can apply that intent to the devices through the device manager. But how do we actually get from a service intent to a device configuration? That is where [**transforms**](https://stratoweave.guide/developer/transforms/) and [**transform functions**](https://stratoweave.guide/developer/transforms/#the-transform-function) come in.

**A transform function takes YANG-modeled input and transforms it into YANG-modeled output for the layer below.** You, as a service developer, can express exactly how the input should be mapped to the output, including any necessary computations, validations, resource allocation, or conditional logic. The **transaction engine** coordinates configuration updates through the layers, so each individual transform only needs to focus on its specific mapping logic.

Transform functions by themselves are great for straightforward mappings. But sometimes you require more advanced state management and coordination, including multi-step procedures and complex workflows. That's where [**transform actors**](https://stratoweave.guide/developer/transforms/#the-transform-actor) come in. These actors can maintain procedure state, coordinate multiple steps, and determine the next action from telemetry updates and execution results. This keeps workflow logic alongside the service models it operates on.
...
Since you have a unified interface for both the intent (desired state) and actual operational state, you can leverage StratoWeave to implement advanced automation and orchestration strategies that respond dynamically to changes in the network environment, true closed-loop automation becomes possible!
