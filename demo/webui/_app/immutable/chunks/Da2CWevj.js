...
`)||`<!-- running and target configuration are in sync -->`);if(s===`log`)return I({log:o.log})}return B(`unknown API path /${e.join(`/`)}`)}function Te(){return(async(e,t)=>{let n=typeof e==`string`?e:e instanceof URL?e.href:e.url,r=new URL(n,`http://demo.invalid`),i=(t?.method??(e instanceof Request?e.method:`GET`)).toUpperCase(),a=new Headers(t?.headers).get(`accept`)??``;return await F(),r.pathname.startsWith(`/restconf/`)?$(r.pathname.slice(10).split(`/`).filter(Boolean),i,t,r.searchParams):we(r.pathname.split(`/`).filter(Boolean),i,t,r.searchParams,a)})}var Ee=Te();export{he as n,Ee as t};